...
const { ParkingZone, Violation, ParkingSlot, User, sequelize } = require('../models');
...
// Create new parking zone (admin and contractor only)
router.post('/', authenticate, authorize('admin', 'contractor'), async (req, res, next) => {
  try {
    const zoneData = {
      ...req.body,
...
// Update parking zone (officer, admin, and contractor)
router.put('/:id', authenticate, authorize('admin', 'officer', 'contractor'), async (req, res, next) => {
  try {
    const zone = await ParkingZone.findByPk(req.params.id);
...
// Delete parking zone (admin and contractor)
router.delete('/:id', authenticate, authorize('admin', 'contractor'), async (req, res, next) => {
  try {
    const zone = await ParkingZone.findByPk(req.params.id);
    ...
    // specific check for contractors
    if (req.user.role === 'contractor' && zone.contractorId !== req.user.id) {
        return res.status(403).json({
            success: false,
            message: 'You can only delete your own zones'
        });
    }
